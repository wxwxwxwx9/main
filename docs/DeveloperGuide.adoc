...
The command `list` allows the user to view the list of unarchived internship application(s) and `archival` allows the user to view the archived list of internship application(s). From here on, we will refer to the unarchived list as "main list".
...
As seen from the class diagram, our implementation involves maintaining 2 `UniqueInternshipApplicationList` objects: `unarchivedInternships` and `archivedInternships`.

`displayedInternships` serves as a reference for either `archivedInternships` or `unarchivedInternships` at any one time.
As the name suggests, it is the current list that is shown to the user in the GUI.
...
The following code snippet is retrieved from the InternshipDiary class. It illustrates the internal workings of how we switch the view between archived list and the main list.
...
However, such implementation brings about issues with reactivity where the UI elements that references `displayedInternships` are not aware of the
reference update whenever the user executes `archival` or `list`. Therefore, users would still see the main list after executing the `archival` command.

To resolve this issue, we employed the observer pattern. The idea is to assign each UI element to be an `observer` and `InternshipDiary` to be the `observable`.
...
To achieve this observer pattern, we implemented the `PropertyChangeListener` interface.

image::InternshipDiaryAndModelManagerPropertyChangeClassDiagram.png[align="center"]

The updated class diagram above illustrates the implementation of the aforementioned observer pattern.

Note that certain methods and members from the previous class diagram have been omitted for brevity. The updated class diagram
shows the additional methods and members.

With reference to the code snippet above, the line of code, `firePropertyChange(DISPLAYED_INTERNSHIPS, getDisplayedInternshipList());`,
is the mechanism that updates the relevant UI elements about the reference change in `displayedInternships`.
