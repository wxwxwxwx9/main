...
The following sequence diagram illustrates how an `archival` command is executed. The sequence diagram for the `list` command is identical.
You may use the same sequence diagram as reference for how the `list` command is carried out.
...
Therefore, we decided to specifically create a new category, "removal-based commands", to classify commands like `archive`, `unarchive`, and `delete`.
This is because the utility of such functions serve to modify the list by removing a certain item. This makes the three commands very similar in terms of functionality.

The intention of introducing such a category is to extend the functionality of such "removal-based commands" to allowing users to execute the commands with different
arguments depending on their intention.
...
The reminder command displays to users a list of internship applications which need to be submitted or have interviews
scheduled in 7 days.
...
The following subsections will go through the general implementations of the reminder command.
...
The two predicates classes implementing `Predicate<InternshipApplication>` are used as per follows:
...
A comparator `ApplicationDateAndInterviewDateComparator` implementing `Comparator<InternshipApplication>` is also used
to sort internship applications in terms of which application is more urgent. It compares internship applications
by their `ApplicationDate` field as well as the earliest interview date in the `ArrayList<Interview> interviews` of the
...
interviews` of the applications in order of which interview comes first.
