...
This feature allows users to view relevant metrics using the data entered by the users for their internship application(s).

Currently, the tracked metrics include:

* the amount of internship applications sent
* percentage of internship applications in each status

==== Implementation

The following class diagram shows the implementation of the statistics feature.

image::StatisticsClassDiagram.png[width="500", align="center"]

Users will be able to view those metrics from two areas:

. `StatisticsBarFooter`
.. found at the bottom of the application in the form of a bar footer
.. serves as a quick view of the metrics in counters
. `StatisticsWindow`
...
`StatisticsWindow` and `StatisticsBarFooter` each contain an instance of `Statistics` that helps them compute the relevant statistics
...
The following activity diagram illustrates how `StatisticsWindow` (`StatisticsBarFooter` has the same workflow) is notified of the updates
in the internship application list and how it updates the statistics.
