...
The following class diagram will illustrate the structure of SortCommand:
...
.Interactions Inside the Logic Component for `InterviewAddCommand`
...
.Interactions Inside the Logic Component for `archival` Command
...
.Activity diagram for Observer Pattern for `archival` Command
...
.Updated class diagram for the implementation of two-tier observer-observable structure with `ListenerPropertyType` enumeration
...
.Structure of `RemovalBasedCommand` with its associated classes
...
.Sequence Diagram of how `RemovalBasedCommandExecuteTypeParser` parses input and determines execution type of command
...
.Creation of lambda function by `RemovalBasedCommand` instance
...
.Execution of removal-based command on one index
...
.Execution of `ArchiveCommand` on indices
...
.Re-creating command result in `RemovalBasedCommand`
...
.Activity Diagram of how `StatisticsWindow` is notified of updates list and statistics is updated
...
The following class diagram shows the relationship between `ReminderCommand`, the `Predicate` classes as well as the
`Comparator` class it uses to display relevant applications:

.Class diagram to show relationship between `ReminderCommand` and the `Predicate` and `Comparator` classes used
image::ReminderClassDiagram.png[]

Included in the diagram above, the several predicates classes implementing `Predicate<InternshipApplication>` used are:
...
A comparator `ApplicationDateAndInterviewDateComparator` implementing `Comparator<InternshipApplication>`, also included in Figure,
is then passed into the method `updateFilteredInternshipApplicationList()` of the `ModelManager` instance to sort internship
...
There was a need to plan how `find`, `sort`, and `reminder` command should behave and how we can extend such commands with new behaviours easily in the future.
...
not foresee). As this is our first time dealing with the observer pattern, we had to spend a good amount of time to understand and implement it properly.
