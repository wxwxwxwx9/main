...
This may be time-consuming in terms of implementation and refactoring of the existing codebase.
...
The following activity diagram depicts the flow of an `archive` command. The activity diagram for `unarchive` is very similar as well.
...
Therefore, for brevity, we will only be illustrating the usage of the command `archive` with the execution type `BY_FIELD`.
...
** Pros: Code will possibly be more elegant and reusable.  +
