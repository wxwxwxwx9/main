...
// tag::find[]
=== Locating internship applications by fields: `find`
...
* `find Google` +
Displays internship application(s) with company `Google` OR `Google Deepmind` OR with email `alice@google.com`

* `find c/Google s/applied` +
Displays internship application(s) with company `Google` AND status `APPLIED`
// end::find[]
...
Delete specific internship applications from the current list. +
...
Archive specific internship applications in the main list. +
...
Unarchive specific internship applications in the archival list. +
